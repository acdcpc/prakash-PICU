...
function TeddyBearBrowser() {
  ...
  useEffect(() => {
    ...
    async function loadApprovalStates() {
      const { data, error } = await supabase.from('teddy_bear_monographs').select('source_id,review_status').eq('review_status', 'approved').limit(2000);
      if (!active) return;
      if (error) {
        setApprovalState('unavailable');
        return;
      }
      setApproved(new Map((data || []).map((row) => [row.source_id, row.review_status])));
  ...
  return <div className="card teddy-catalog-card"><div className="card-head"><div><span className="eyebrow">Complete authorized index</span><h3>All Teddy Bear monographs</h3><p className="text-muted">{TEDDY_BEAR_REVIEW_INDEX.length.toLocaleString()} full drug monographs from {TEDDY_BEAR_REVIEW_SOURCE.source}. Each requires clinical verification before numeric use.</p></div><BookOpen size={24} /></div><div className="card-body"><div className="notice notice-info"><BookOpen size={17} /><span>Every authorized full drug monograph is visible here. Full text remains authenticated in Supabase. Only records explicitly verified by the clinical team may be promoted into a numeric calculator.</span></div>{approvalState === 'unavailable' && <div className="notice notice-warning"><ShieldCheck size={17} /><span><strong>Approval status unavailable:</strong> Supabase review status could not be loaded. Treat every record as unverified until the secure table connection is restored.</span></div>}<div className="catalog-toolbar"><div className="search-field"><Search size={16} /><input className="form-input" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search all Teddy Bear headings" /></div><select className="form-select catalog-status" value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">All indexed records</option><option value="approved">Approved review</option><option value="pending">Pending review</option></select></div><div className="catalog-summary"><span>{filtered.length.toLocaleString()} matching records</span><span className="text-muted">Page {page} of {totalPages}</span></div><div className="teddy-catalog-list">{visible.map((item) => <a className="teddy-catalog-row" key={item.id} href={`/teddy-bear-review?source_id=${encodeURIComponent(item.id)}`}><span className="catalog-icon"><BookOpen size={15} /></span><span><strong>{item.name}</strong><small>{approvalState === 'unavailable' ? 'Approval status unavailable · open authenticated review' : approved.has(item.id) ? 'Approved review · open authenticated monograph' : 'Pending clinical verification · review required before calculation'}</small></span><ArrowRight size={16} /></a>)}</div><div className="catalog-pagination"><button className="btn btn-ghost btn-sm" disabled={page <= 1} onClick={() => setPage((current) => current - 1)}>Previous</button><button className="btn btn-ghost btn-sm" disabled={page >= totalPages} onClick={() => setPage((current) => current + 1)}>Next</button></div><a className="btn btn-primary mt-3" href="/teddy-bear-review">Open full authenticated review workspace <ArrowRight size={15} /></a></div></div>;
}
