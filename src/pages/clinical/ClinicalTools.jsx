...
function ScalePanel({ type, context }) {
  ...
  const categorical = Array.isArray(definition.options);
  return <div className="card"><div className="card-head"><div><h3>{definition.title}</h3><p className="text-muted">{definition.interpretation}</p></div><span className="badge badge-blue">{definition.range}</span></div><div className="card-body"><div className="score-context">{context.age ? `${context.age} ${context.ageUnit}` : 'Age not entered'} · {context.weight ? `${context.weight} kg` : 'Weight not entered'} · {context.sex || 'sex not specified'}</div>{categorical ? <div className="scale-options">{definition.options.map((option) => <button key={option.value} type="button" className={`scale-option ${Number(scores.selected) === option.value ? 'selected' : ''}`} onClick={() => setScores({ selected: option.value })}><span className="scale-option-value">{option.value > 0 ? `+${option.value}` : option.value}</span><span>{option.label}</span></button>)}</div> : definition.items.map((item, index) => <div className="score-row" key={item}><label>{index + 1}. {item}</label><input className="form-input score-input" type="number" min={definition.min ?? 0} max={definition.max ?? 5} value={scores[index] || ''} onChange={(e) => setScores((current) => ({ ...current, [index]: e.target.value }))} placeholder="Score" /></div>)}<div className="score-total"><span>{categorical ? 'Selected state' : 'Total recorded score'}</span><strong>{categorical ? (scores.selected === undefined ? '—' : `${scores.selected > 0 ? '+' : ''}${scores.selected}`) : total}</strong></div><div className="notice notice-warning mt-3"><AlertTriangle size={16} /><span>Use the validated instrument, age-appropriate observation, prescribed target, and local threshold. This screen is a documentation aid, not a standalone diagnosis or sedation order.</span></div><ReferenceLink id={definition.reference} /></div></div>;
}
...
export default function ClinicalTools() {
  ...
  return <div className="page-content clinical-page"><div className="page-heading"><div><p className="eyebrow">Pediatric bedside workspace</p><h2>Drugs & Scores</h2><p className="text-muted">Dose references, sedation and pain scores, delirium screens, POCUS prompts, and care pathways for children.</p></div></div><PatientContext context={context} setContext={setContext} /><div className="clinical-tabs">{TABS.map(([id, label]) => <button key={id} className={activeTab === id ? 'active' : ''} onClick={() => setActiveTab(id)}>{label}</button>)}</div><div className="clinical-panel">{activeTab === 'drugs' && <DrugPanel context={context} />}{activeTab === 'scores' && <div><div className="scale-switcher">{['COMFORT-B', 'PSSS', 'RASS', 'SBS'].map((scale) => <button key={scale} className={activeTab === scale ? 'active' : ''} onClick={() => setActiveTab(scale)}>{scale}</button>)}</div><ScalePanel type="COMFORT-B" context={context} /></div>}{['COMFORT-B', 'PSSS', 'RASS', 'SBS'].includes(activeTab) && <div><div className="scale-switcher">{['COMFORT-B', 'PSSS', 'RASS', 'SBS'].map((scale) => <button key={scale} className={activeTab === scale ? 'active' : ''} onClick={() => setActiveTab(scale)}>{scale}</button>)}</div><ScalePanel type={activeTab} context={context} /></div>}{activeTab === 'pain' && <PainPanel context={context} />}{activeTab === 'delirium' && <DeliriumPanel context={context} />}{activeTab === 'pocus' && <PocusPanel />}{activeTab === 'algorithms' && <AlgorithmsPanel context={context} />}</div><div className="clinical-footnote"><strong>Verification is required.</strong> Clinical content must be reconciled with current institutional protocols, licensed references, patient-specific data, and the supervising clinician’s judgment. <span>References:</span> {CLINICAL_REFERENCES.map((reference) => <a key={reference.id} href={reference.url} target="_blank" rel="noreferrer">{reference.label}</a>)}</div></div>;
}
